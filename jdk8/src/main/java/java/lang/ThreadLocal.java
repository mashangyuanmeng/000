...
public class ThreadLocal<T> {
    ...
    public T get() {
        ...
        if (map != null) {
            /**
             * 判断是否存在当前线程的副本，存在则返回副本，不存在则存储进去，并将其返回
             * 下面注意这里获取键值对传进去的是 this，而不是当前线程t，所以获取的是当前
             */
    ...
    /**
     * 内部类；
     * 没有实现Map接口，用独立的方式实现了Map的功能，其内部的Entry也独立实现
     */
    static class ThreadLocalMap {

        /**
         * Entry 继承自 WeakReference（弱引用，生命周期只能存活到下次GC前），但只有Key是弱引用类型的，Value并非弱引用。
         * 会导致如下问题：
